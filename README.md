...
| 2 | Ansible 플레이북 <br/>*(이전 개인 프로젝트)* | 약 3분 (180초) | **95.0% 단축** |
| 3 | **Custom AMI + SSM join** <br/>***(이번 프로젝트)*** | **56초** | **98.4% 단축** |

Ansible이 수동 설치를 95% 줄여줬지만, ASG 앞에서는 그 3분조차 길었습니다.
실행 시점을 사람이 정해야 한다는 전제가 오토스케일링과 맞지 않았고, 스케일아웃은
이미 부하가 몰린 뒤에 일어나므로 패키지 설치에 쓰는 몇 분이 그대로 지연으로 남기 때문입니다.
Custom AMI는 그 3분을 다시 **68.9% 단축**해 56초로 만들었습니다.
...
부하가 사라지자 Cluster Autoscaler가 같은 노드를 되돌립니다.
`SchedulingDisabled`(cordon)로 신규 배치를 막고 파드를 비운 뒤 EC2를 종료합니다.

![ASG 노드 축소](docs/result-autoscale-in.png)

노드에 남은 CA taint 값이 유닉스 타임스탬프라, 판단 시각이 초 단위로 남습니다.

| 시각 (UTC) | 사건 |
|---|---|
| 13:22:10 | EC2 부팅 |
| 13:23:06 | 노드 `Ready` — **증설 56초** |
| 13:23:15 | `DeletionCandidateOfClusterAutoscaler` — 불필요 판정 |
| 13:33:17 | `ToBeDeletedByClusterAutoscaler` — 삭제 확정 (`scale-down-unneeded-time=10m` 경과) |
| 13:33:54 | EC2 종료 — **축소 10분 48초** |

**늘리는 데 56초, 줄이는 데 약 11분.** 이 비대칭은 의도한 것입니다.
스케일아웃은 이미 부하가 몰린 뒤에 일어나므로 빨라야 하고, 스케일인은 성급하면
곧바로 다시 늘리는 플래핑이 나 EC2 요금만 늘기 때문에 느려야 합니다.
...
| 지표 | 값 |
|---|---|
| 노드 합류 시간 (EC2 부팅 → `Ready`) | **56초** — 수동 대비 **98.4% 단축**, Ansible 대비 **68.9% 단축** |
| 열린 인바운드 포트 | **0개** |
| Terraform 모듈 | **10개** |
| 노드 축소 시간 (`Ready` → EC2 종료) | **10분 48초** (플래핑 방지를 위한 의도적 지연) |
